function onGoogleSignIn(googleUser) {
	...
	$.get("/login_google/"+profile.getEmail() + "+" + profile.getName())
  .done(function () {
    GoogleSignOut();
...
//                 console.log('authResponse ' + Object.keys(response.authResponse) + '.');
                ...
                FB.api('/me?fields=name,id,email', function(response) {
	                ...
	                glb_email = response.email;
					console.log('User Name is, ' + glb_uname + '.');
       				console.log('User Facebook ID is, ' + glb_uid + '.');
       				console.log('User Facebook email is, ' + glb_email + '.');
       				$.get("/login_facebook/" + glb_uname + "+" + glb_email)
       				.done(function () {
    					FB.logout();
    					window.location.replace("/");
  					});
